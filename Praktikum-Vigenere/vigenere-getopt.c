/*
  ...
  C:\Program Files (x86)\GnuWin32\bin\gengetopt.exe --include-getopt --unamed-opts -i vigenere-getopt.ggo -F vigenere-getopt 
...
const char *gengetopt_args_info_usage = "Usage: vigenere [OPTIONS]... [FILES]...";
...
cmdline_parser_release (struct gengetopt_args_info *args_info)
{
  ...
  unsigned int i;
  if (args_info->key_arg)
    {
      free (args_info->key_arg); /* free previous argument */
      args_info->key_arg = 0;
    }
  if (args_info->key_orig)
    {
      free (args_info->key_orig); /* free previous argument */
      args_info->key_orig = 0;
    }
  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case 'k':	/* Key to be used for encryption or decryption.  */
          if (local_args_info.key_given)
            {
              fprintf (stderr, "%s: `--key' (`-k') option given more than once%s\n", argv[0], (additional_error ? additional_error : ""));
              goto failure;
            }
          if (args_info->key_given && ! override)
            continue;
          local_args_info.key_given = 1;
          args_info->key_given = 1;
          if (args_info->key_arg)
            free (args_info->key_arg); /* free previous string */
          args_info->key_arg = gengetopt_strdup (optarg);
          if (args_info->key_orig)
            free (args_info->key_orig); /* free previous string */
          args_info->key_orig = gengetopt_strdup (optarg);
          break;
        ...
        case 'e':	/* Encryption.  */
          if (local_args_info.encrypt_given)
            {
              fprintf (stderr, "%s: `--encrypt' (`-e') option given more than once%s\n", argv[0], (additional_error ? additional_error : ""));
              goto failure;
            }
          if (args_info->encrypt_given && ! override)
            continue;
          local_args_info.encrypt_given = 1;
        ...
        case 'd':	/* Decryption.  */
          if (local_args_info.decrypt_given)
            {
              fprintf (stderr, "%s: `--decrypt' (`-d') option given more than once%s\n", argv[0], (additional_error ? additional_error : ""));
              goto failure;
            }
          if (args_info->decrypt_given && ! override)
            continue;
          local_args_info.decrypt_given = 1;
