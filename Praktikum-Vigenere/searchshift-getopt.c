/*
  ...
  C:\Program Files (x86)\GnuWin32\bin\gengetopt.exe --include-getopt --unamed-opts -i searchshift-getopt.ggo -F searchshift-getopt 
...
const char *gengetopt_args_info_usage = "Usage: searchshift [OPTIONS]... [FILES]...";
...
cmdline_parser_release (struct gengetopt_args_info *args_info)
{
  ...
  unsigned int i;
  if (args_info->columns_orig)
    {
      free (args_info->columns_orig); /* free previous argument */
      args_info->columns_orig = 0;
    }
...
cmdline_parser_file_save(const char *filename, struct gengetopt_args_info *args_info)
{
  ...
  if (args_info->columns_given) {
    if (args_info->columns_orig) {
      fprintf(outfile, "%s=\"%s\"\n", "columns", args_info->columns_orig);
    } else {
      fprintf(outfile, "%s\n", "columns");
    }
  }
  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case 'c':	/* Number of columns to be analyzed..  */
          if (local_args_info.columns_given)
            {
              fprintf (stderr, "%s: `--columns' (`-c') option given more than once%s\n", argv[0], (additional_error ? additional_error : ""));
              goto failure;
            }
          if (args_info->columns_given && ! override)
            continue;
          local_args_info.columns_given = 1;
          args_info->columns_given = 1;
          args_info->columns_arg = strtol (optarg, &stop_char, 0);
          if (!(stop_char && *stop_char == '\0')) {
            fprintf(stderr, "%s: invalid numeric value: %s\n", argv[0], optarg);
            goto failure;
          }
          if (args_info->columns_orig)
            free (args_info->columns_orig); /* free previous string */
          args_info->columns_orig = gengetopt_strdup (optarg);
          break;
